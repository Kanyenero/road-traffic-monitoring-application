...
namespace Rsreu.Diploma.VideoProcessing.Sort.Kalman;
...
internal class Matrix
{
    ...
    public Matrix(int[,] values) : this(values.GetLength(0), values.GetLength(1))
    {
    ...
    private (double[,] Result, int[] Indices, double D) GetDecomposition()
    {
        ...
        for (var col = 0; col < Columns; col++)
        {
            ...
            if (col != max_row)
            {
                ...
                {
                    (result[col, k], result[max_row, k]) = (result[max_row, k], result[col, k]);
                }
    ...
    internal class MatrixDisplay
    {
        public MatrixDisplay(Matrix matrix)
        {
            Cells = Enumerable.Range(0, matrix.Rows)
                .Select(row => new Cell(string.Join("  ", Enumerable.Range(0, matrix.Columns).Select(col => matrix._values[row, col]))))
                .ToArray();
        }
